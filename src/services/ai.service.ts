...
export const feedbackRoadmap = async (userId: string, reqBody: RoadmapFeedbackSchema) => {
  ...
  const userRoadmap = await UserRoadmap.findById(userRoadmapId).lean()
  if (!userRoadmap) {
  ...
  const edittedTopic = await MasterTopic.findById(topicId).lean()
  if (!edittedTopic) {
  ...
  edittedTopic.dependsOn?.requiredTopicIds?.forEach((id) => allRequiredIds.add(id.toString()))
  ...
  const resolveNames = (ids: Types.ObjectId[] = []) =>
    ids.map((id) => idToNameMap.get(id.toString()) || id.toString())
  ...
  const feedbackInput: RoadmapFeedbackInput = {
    ...
    action: action,
    editedTopic: {
      name: edittedTopic.name,
      descriptionShort: edittedTopic.descriptionShort,
      prerequisiteNames: resolveNames(edittedTopic.dependsOn?.requiredTopicIds),
    },
    currentTopics: userTopics.map((userTopic) => {
